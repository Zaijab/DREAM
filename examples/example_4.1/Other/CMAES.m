...
%Initialization
strfitnessfct = 'f';  %name of objective/fitness function
N =2;                 %number of objective variables/problem dimension
xmean = rand(N,1);    %objective variables initial point
sigma = 0.5;          %coordinate wise standard deviation (step size)
stopfitness =-1000;      %stop if fitness < stopfitness (minimization)
...
%Block-wise sampler initialization
nS=lambda*1e3;                      %Number of samples
nB=nS*0.1;                          %Burn in 
t=1;                                %MCMC steps  
x=NaN(nS/lambda,N,lambda);                 %Estimands
x(1,:,:)=randn(1,N,lambda);         %First set of estimands
...
while counteval < nS
    ...
    arx=NaN(N,lambda);
    arfitness=NaN(1,lambda);
    t = t + 1;                          %xStar index
    for k=1:lambda,
        arx(:,k) = xmean + sigma * B * (D .* randn(N,1)); % m + sig * Normal(0,C)
        arfitness(k) = feval(strfitnessfct, arx(:,k)',k); % objective function call
